...
const ChatInbox = () => {
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        ...
        `${API}/api/chat-session/user-sessions/${user.email}`
  ...
  const startNewChat = async () => {
    ...
    try {
      ...
      // ✅ Balance kam hai → recharge modal
  ...
  // ✅ Recharge (Razorpay) → phir dobara start
...
<div
      ...
      className="w-full text-white flex justify-center px-2 lg:px-6 pb-4"
...
<div
          ...
          className={`flex-col border-r border-purple-800/40 bg-[#14141f] w-full md:w-[340px] md:shrink-0 ${
...
<span className="text-xs bg-purple-700/50 px-2 py-1 rounded-full text-purple-200">
...
<p className="font-semibold truncate text-sm">
                      ...
                      {s.astrologerId?.name || "Astrologer"}
</p>
...
<p className="text-xs mt-2 text-gray-600">
                  ...
                  Kisi chat pe click karo — messages yahan khulenge
</p>
</div>
</div>
...
<button
                  ...
                  className="md:hidden text-purple-300 text-xl mr-1"
>
...
<p className="font-bold text-sm">
                    ...
                    {selected.astrologerId?.name || "Astrologer"}
</p>
<p

                    className={`text-[11px] ${

                      selected.status === "active" ? "text-green-400" : "text-gray-500"
...
<div
                ...
                style={{
                  ...
                  backgroundImage:
                    ...
                    "radial-gradient(rgba(150,80,255,0.06) 1px, transparent 1px), radial-gradient(rgba(255,80,180,0.05) 1px, transparent 1px)",
...
<p className="text-[9px] text-gray-400 text-right mt-1">
                          ...
                          {isOwn && (
<span className={msg.seen ? "text-cyan-400 font-bold" : ""}> ✓✓</span>
...
<div className="bg-[#1e1e2c] rounded-xl px-4 py-2 text-xs text-purple-300 italic">
